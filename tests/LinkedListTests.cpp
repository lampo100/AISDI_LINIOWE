...
/******************************************MOJE TESTY**********************************************/
    BOOST_AUTO_TEST_CASE_TEMPLATE(GivenTwoCollections_WhenGivingIteratorToOther_ThenThrow,
                                  T,
                                  TestedTypes)
    {
        LinearCollection<T> collection ={1, 2, 3, 4, 5, 6, 7};
        LinearCollection<T> collection2 ={1, 2, 3, 4, 5, 6, 7};

        auto it = collection.begin()+3;
        auto it2 = collection.begin() + 5;
        auto it3 = collection2.begin();

        BOOST_CHECK_THROW(collection2.erase(it),std::out_of_range);
        BOOST_CHECK_THROW(collection2.erase(it,it2),std::out_of_range);
        BOOST_CHECK_THROW(collection2.erase(it3,it2),std::out_of_range);
    }
    BOOST_AUTO_TEST_CASE_TEMPLATE(GivenNonEmptyCollection_WhenShiftingIteratorViaPlusOperator_withNegativeNumber,T,TestedTypes)
    {
        LinearCollection<T> collection ={100, 23, 434, 34,4224 ,434, 432423, 423,423, 5,435 ,43,5 ,534};
        auto it = collection.begin()+3;
        auto it2 = it + (-3);
        BOOST_CHECK(it2 == collection.begin());
        BOOST_CHECK_THROW(it2+(-30),std::out_of_range);
    }
    BOOST_AUTO_TEST_CASE_TEMPLATE(GivenNonEmptyCollection_WhenShiftingIteratorViaMinusOperator_withNegativeNumber,T,TestedTypes)
    {
        LinearCollection<T> collection = {1, 2, 3, 4, 5, 6, 7, 8, 9};
        auto it = collection.begin()+3;
        auto it2 = it - (-3);
        BOOST_CHECK_EQUAL(*it2, 7);
    }
